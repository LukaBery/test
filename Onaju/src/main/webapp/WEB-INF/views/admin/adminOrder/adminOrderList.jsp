...
<div>
	<form>
		<div class="host-search-1">
			...
			<div>
			<label for="startDate">시작날짜&nbsp;:&nbsp;</label><input class="datepicker" id="startDate" name="cre_startDate" autocomplete=off>&emsp;
				<label for="endDate">종료날짜&nbsp;:&nbsp;</label><input class="datepicker" id="endDate" name="cre_endDate" autocomplete=off>&emsp; 
		...
		<div class="host-search-1">
			...
			<div class="host-row-col3"><div><select><option value="">상태</option><option value="가입완료">입금완료</option><option value="입금대기">입금대기</option><option value="예약환불">예약환불</option></select></div></div>
...
<div>
	...
	<c:if test="${pageMaker.prev }">
		<div>
			<a href="${contextPath }/admin/orderList.do?page=${pageMaker.startPage - 1}">앞으로</a>
		</div>
	</c:if>
	<c:forEach var="pageNum" begin="${pageMaker.startPage }" end="${pageMaker.endPage }">
		<div style="width: 30px; height: 30px; margin: 15px 3px 15px 3px; border: 1px solid #eeeeee; border-radius:5px; text-align: center; line-height: 30px;">
			<a style="text-decoration: none; color: #666666;" href="${contextPath }/admin/orderList.do?page=${pageNum}">${pageNum }</a>
		</div>
	</c:forEach>
	<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
		<div>
			<a href="${contextPath }/admin/orderList.do?page=${pageMaker.endPage + 1}">뒤로</a>
		</div>
	</c:if>
	</div>
	</div>
